...
JS подключаем в dev.html просто как <script src="http://localhost:3003/HuEvaFlowEnhancer.js">, а в tampermonkey скрипте `evateam-workflow-enhance.user.js` через `@require` по этому же адресу пока.
...
2. Вотчинг запущен (не надо запускать), можно смотреть и тестировать http://localhost:3003/
